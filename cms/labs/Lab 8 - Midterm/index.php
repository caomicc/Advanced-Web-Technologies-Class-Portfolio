        ...
        <div id="page-content-wrapper">
            <div class="container-fluid">
               <!--<div class="menu-div"><a href="#menu-toggle" class="btn btn-default" id="menu-toggle"><span class="glyphicon glyphicon-menu-hamburger" aria-hidden="true"></span></a></div>-->
                ...
                <div class="row">
                <div class="col-lg-12">
<h2>State List and Search  <small>Midterm</small></h2><br>
...
$condition = "where state_id > -1";
// add additional conditions
$condition .= " and (state_name like '%$searchName%' )";
...
$query = "SELECT state_id, state_name, state_abbr, regionTab.regionName
FROM stateTab
INNER JOIN regionTab ON stateTab.region = regionTab.regionID
WHERE state_name LIKE '%{$searchName}%' 
ORDER BY state_id
limit $pstart, $scroll_amt;";
$result = mysql_query($query) or die(mysql_error());
...
if ($pstart != 0) {
echo '<li class="previous"><a href="index.php?pcode=b&pstart=' . $back . '&pmax=' . $pmax . '&searchName=' . $searchName . '">&laquo;Back</a></li>';
} // end it

// format forward button using forward amount as start
// only display if there are recs to display
if (($pstart + $scroll_amt) < $pmax) {
echo '<li class="next"><a href="index.php?pcode=f&pstart=' . $forward . '&pmax=' . $pmax . '&searchName=' . $searchName . '">Next&raquo;</a></li>';
} // end it
...
<input type=button onClick="parent.location='index.php'" class="btn btn-sunset" value='Return'>
                     </div>
                </div>
            </div>
            </div>
